...
def evaluate_on_test_parallel(net, epoch, params_path, test_dataset, mean_node, std_node, mean_edge, std_edge,
    ...
    with torch.no_grad():
        ...
        for step in steps:
            ...
            print('MAPE:', MAPE(true_edge[:, step - 1], prediction_edge[:, step - 1]) / 10.0)
        return true_edge, prediction_edge


def evaluate_on_test_parallel_acc(net, epoch, params_path, test_dataset, mean_node, std_node, mean_edge, std_edge,
                                  batch_size, DEVICE):
    '''
    :param global_step:
    :param test_loader_node:
    ...
    with torch.no_grad():
        ...
        for batch_index in range(batch_test):
            ...
            prediction_edge.extend(out_edge.cpu().numpy().tolist())
            true_edge.extend(test_target_edge_tensor.cpu().numpy().tolist())
            acc.extend(test_accident_tensor.cpu().numpy().tolist())
        prediction_node = np.array(prediction_node)
        true_node = np.array(true_node)
        prediction_edge = np.array(prediction_edge)
        acc = np.array(acc)
...
def filter_acc(test_dataset):
    ...
    test_x_node_tensor, test_x_edge_tensor, test_accident_tensor, test_x_sub_edge_tensor, test_target_node_tensor, test_target_edge_tensor = test_dataset[
                                                                                                                                             :]
    test_x_edge_tensor=torch.from_numpy(np.load('/public/lhy/wmy/tmp/x_test.npy')).permute(0,2,1).unsqueeze(dim=3)[:6908]
    test_target_edge_tensor = torch.from_numpy(np.load('/public/lhy/wmy/tmp/y_test.npy')).permute(0,2,1).unsqueeze(dim=3)[:6908]
    di = 0
    # 遍历样本
