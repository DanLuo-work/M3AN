...
import numpy as np
from lib.utils import load_graphdata, compute_val_loss_parallel, evaluate_on_test_parallel, filter_acc,evaluate_on_test_parallel_acc
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        evaluate_on_test_parallel(net, epoch, params_path, test_dataset, mean_node, std_node, mean_edge, std_edge,
                                  batch_size, DEVICE)
        print('acc:')
        test_dataset_acc = filter_acc(test_dataset)
        evaluate_on_test_parallel(net, epoch, params_path, test_dataset_acc, mean_node, std_node,
                                                               mean_edge, std_edge,
                                                               batch_size, DEVICE)
...
def filter_acc(test_dataset):
    ...
    di = 0
    # 遍历样本
    l = len(test_accident_tensor)
    test_accident_tensor_copy = test_accident_tensor
    for i in range(l):
        if np.sum(test_accident_tensor_copy[i, 0].numpy()) < 1:
            # print(di)
            test_x_node_tensor = test_x_node_tensor[torch.arange(test_x_node_tensor.size(0)) != i - di]
            test_x_edge_tensor = test_x_edge_tensor[torch.arange(test_x_edge_tensor.size(0)) != i - di]
            test_accident_tensor = test_accident_tensor[torch.arange(test_accident_tensor.size(0)) != i - di]
            test_x_sub_edge_tensor = test_x_sub_edge_tensor[torch.arange(test_x_sub_edge_tensor.size(0)) != i - di]
            test_target_node_tensor = test_target_node_tensor[torch.arange(test_target_node_tensor.size(0)) != i - di]
            test_target_edge_tensor = test_target_edge_tensor[torch.arange(test_target_edge_tensor.size(0)) != i - di]
            di += 1
    test_dataset = torch.utils.data.TensorDataset(test_x_node_tensor, test_x_edge_tensor,
                                                  test_accident_tensor, test_x_sub_edge_tensor,
                                                  test_target_node_tensor, test_target_edge_tensor)
    np.save('./test_accident.npy', test_accident_tensor.numpy())
    return test_dataset
if __name__ == "__main__":
    # train_main()
    test_dataset = filter_acc(test_dataset)
    true_edge, prediction_edge = evaluate_on_test_parallel(net, 21, params_path, test_dataset, mean_node, std_node,
                                                           mean_edge, std_edge,
                                                           batch_size, DEVICE)
    np.save('/public/lhy/wmy/tmp/true_edge_SubMAGCN_fil.npy', true_edge)
    np.save('/public/lhy/wmy/tmp/prediction_edge_SubMAGCN_fil.npy', prediction_edge)
